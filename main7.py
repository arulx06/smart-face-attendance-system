...
def maybe_reload_encodings():
    ...
    try:
        ...
        log(f"Encoding reload error: {e}")


def run_encoder():
    try:
        log("🔄 Running EncodeGenerator...")
        os.system("python EncodeGenerator.py")
        log("✅ Encodings updated")
    except Exception as e:
        log(f"Encoding generation failed: {e}")
...
def tcp_server():
    ...
    while True:
        try:
            ...
            # If registration was happening, still run encoder
            if pi_state == PiState.REGISTER_ACTIVE or pi_state == PiState.REGISTER_PREP:
                log("⚠️ TCP disconnected during registration — running encoder")
                threading.Thread(target=run_encoder, daemon=True).start()
