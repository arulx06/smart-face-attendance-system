...
def run_incremental_encoding(student_id):

    log(f"🔄 Encoding new student {student_id}")

    try:
        encode_student(student_id)
        log(f"✅ Encoding finished for {student_id}")
        ...
        try:
            ...
            while True:
                ...
                if cmd == "REGISTER_STOP":
                    ...
                    # background encoding
                    multiprocessing.Process(target=run_incremental_encoding,args=(student_id,),daemon=True).start()
            ...
            # If registration was happening, force encoding
            if pi_state in [PiState.REGISTER_ACTIVE, PiState.REGISTER_PREP]:

                student_id = current_register_id

                if student_id is not None:
                    log(f"⚠️ Registration interrupted — encoding {student_id}")
